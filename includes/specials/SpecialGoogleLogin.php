	...
	private function showSummary( Google_Service_Plus $plus ) {
		...
		if ( !$isLinked ) {
			$out->addHTML( new \OOUI\ButtonWidget( [
				'href' => SpecialPage::getTitleFor( 'LinkAccounts' )->getLocalURL(),
				'label' => $this->msg( 'googlelogin-form-merge' )->text(),
				'flags' => [ 'progressive' ],
			] ) );
		} else {
			$out->addHTML( new \OOUI\ButtonWidget( [
				'href' => SpecialPage::getTitleFor( 'RemoveCredentials' )->getLocalURL(),
				'label' => $this->msg( 'googlelogin-form-unlink' )->text(),
				'flags' => [ 'progressive' ],
			] ) );
		}
	}
