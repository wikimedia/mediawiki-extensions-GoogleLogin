	...
	class SpecialManageGoogleLogin extends SpecialPage {
		...
		function execute( $par ) {
			...
			if ( !$request->getVal( 'glManageableUser' ) ) {
				...
				$htmlForm->setSubmitCallback( array( 'SpecialManageGoogleLogin', 'submitUserName' ) );
				$htmlForm->show();
			} else {
				self::submitUserName(
					array(
		 ...
		 * @return boolean
		 */
		public static function submitUserName( $data ) {
		...
		private function manageUser( User $user ) {
			...
			if ( $id ) {
				$out->addWikiMsg( 'googlelogin-manage-linked', $id['id'] );
				$formId = $id['id'];
			} else {
