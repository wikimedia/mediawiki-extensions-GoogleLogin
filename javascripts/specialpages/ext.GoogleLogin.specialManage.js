$( function ( mw ) {
	...
	mw.GoogleLogin = {
			...
			InfoDialog.prototype.initialize = function () {
				...
				api.get( {
						...
						$.each( data.googleplusprofileinfo.result, function ( index, value ) {
							...
							if ( index !== 'profileimage' ) {
								...
								element = new OO.ui.FieldsetLayout( {
									$: self.$,
									label: mw.GoogleLogin.getMsg( index )
								} );
								// determine if it is a boolean value and use yes/no
								if ( value === true || value === false ) {
									value = (
										value === true ?
										mw.GoogleLogin.getMsg( 'yes' ) :
										mw.GoogleLogin.getMsg( 'no' )
									);
								}
								element.addItems( [
					...
					if ( error.info ) {
						...
						msg = mw.GoogleLogin.getMsg( code, 'googlelogin-manage-unknownerror' );
					} else {
						msg = mw.GoogleLogin.getMsg( 'googlelogin-manage-unknownerror' );
					}
					self.close();
		...
		/**
		 * Maps short message strings to the message keys used in messages file
		 *
		 * @param {String} key The short messag key
		 * @param {String} fallback A fallback message key to use, if the short message key does not exist
		 * @return {String}
		 */
		getMsg: function ( key, fallback ) {
			var msgMap = {
				displayName: 'googlelogin-googleuser',
				isPlusUser: 'googlelogin-manage-isplusser',
				orgName: 'googlelogin-manage-orgname',
				orgTitle: 'googlelogin-manage-orgtitle',
				orgSince: 'googlelogin-manage-orgsince',
				yes: 'googlelogin-manage-yes',
				no: 'googlelogin-manage-no'
			};

			if ( msgMap[key] ) {
				return mw.msg( msgMap[key] );
			} else if ( fallback ) {
				return mw.msg( fallback );
			} else {
				return mw.msg( key );
			}
		}
